import Head from 'next/head';

export default function About() {
  return (
    <>
      <Head>
        <title>About | NoSleep Systems</title>
      </Head>

      <section className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-black text-white px-6 py-24 md:px-16 relative overflow-hidden">
        <div className="absolute inset-0 bg-grid-slate-800/10 pointer-events-none z-0" />
        <div className="max-w-6xl mx-auto z-10 relative">

          <h1 className="text-5xl md:text-6xl font-bold leading-tight tracking-tight mb-6">
            We don’t build websites. <br />
            We install <span className="text-accent">systems</span> that never sleep.
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl mb-12">
            Every asset we deploy is engineered to multiply output, capture missed revenue, and run without you. You bring the vision — we build the machine.
          </p>

          <div className="grid md:grid-cols-2 gap-16 mt-10">
            <div>
              <h2 className="text-2xl font-semibold text-white mb-4">What We Deploy</h2>
              <ul className="text-slate-400 space-y-3">
                <li>✔ Conversion Pages that sell on autopilot</li>
                <li>✔ Lead Capture Funnels that qualify and close</li>
                <li>✔ AI Agents that handle repetitive ops</li>
                <li>✔ Follow-Up Sequences that revive dead leads</li>
                <li>✔ Dashboards that show ROI in real-time</li>
              </ul>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
              <h3 className="text-lg font-bold text-white mb-4">Why We're Different</h3>
              <ul className="text-slate-400 text-sm space-y-2">
                <li>– Built with conversion psychology frameworks</li>
                <li>– Delivered like software, not slow agencies</li>
                <li>– Fully automated — not just pretty design</li>
                <li>– Speed. Precision. No fluff.</li>
              </ul>
            </div>
          </div>

          <div className="mt-24 text-xl text-slate-300 font-light italic border-t border-slate-800 pt-10 max-w-3xl">
            "If your business still runs on manual effort, you’re already behind. We install leverage so you can scale without burnout."
          </div>

          <div className="mt-12">
            <a
              href="/install"
              className="inline-block bg-accent text-black px-6 py-3 rounded-md text-sm font-semibold hover:brightness-110 transition"
            >
              Install My System →
            </a>
          </div>
        </div>
      </section>
    </>
  );
}