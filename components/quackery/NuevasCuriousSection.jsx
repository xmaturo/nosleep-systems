...
export default function NuevasCuriousSection() {
  ...
  const rPct = (clamp(r, 0, MAX_R) / MAX_R) * 100;
  const classicalPct = (REGIME_THRESHOLDS.classicalMax / MAX_R) * 100;
  const crossoverPct = ((REGIME_THRESHOLDS.crossoverMax - REGIME_THRESHOLDS.classicalMax) / MAX_R) * 100;
  const quantumPct = 100 - classicalPct - crossoverPct;

  const regime = useMemo(() => {
    if (r < REGIME_THRESHOLDS.classicalMax) return 'Classical / rate-dominant';
    if (r <= REGIME_THRESHOLDS.crossoverMax) return 'Crossover / high sensitivity';
      ...
      <div className="mt-6 grid gap-5">
        ...
        <div className="border-[3px] border-wine p-3.5 bg-void/20">
          ...
          <div className="relative h-7 border border-wine overflow-hidden">
            <div className="absolute inset-y-0 left-0 bg-gold/15" style={{ width: `${classicalPct}%` }} />
            <div className="absolute inset-y-0 bg-magenta/18" style={{ left: `${classicalPct}%`, width: `${crossoverPct}%` }} />
            <div className="absolute inset-y-0 right-0 bg-ember/15" style={{ width: `${quantumPct}%` }} />
